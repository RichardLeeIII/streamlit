...
def process_uploaded_file(uploaded_file, model):
    if uploaded_file is not None:
        ...
        if set(COLS[1:]).issubset(data.columns):
            # Encode the features
            data_encoded = model.named_steps['encoder'].transform(data[COLS[1:]])

            # Make predictions
            predictions = model.named_steps['gbc'].predict(data_encoded)
...
if uploaded_file is not None:
    pipe = load_model()
    result_df = process_uploaded_file(uploaded_file, pipe)
